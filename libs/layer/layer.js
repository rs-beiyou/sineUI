...
! function(window, undefined) {
  ...
  //缓存常用字符
  let doms = ['layui-layer', '.layui-layer-title', '.layui-layer-main', '.layui-layer-dialog', 'layui-layer-iframe', 'layui-layer-content', 'layui-layer-btn', 'layui-layer-close'];
  ...
  class Modal {
    constructor(options) {
      ...
      that.config = Object.assign({}, Modal.DEFAULTS, ready.config, options);
    ...
    creat() {
      ...
      switch (config.type) {
        case 0:
          ...
          break;
        case 2:
          let content = config.content = conType ? config.content : [config.content || 'http://layer.layui.com', 'auto'];
  ...
  layer.prompt = function(options, yes) {
    ...
    let prompt, content = options.formType == 2 ? '<textarea class="layui-layer-input"' + style + '>' + (options.value || '') + '</textarea>' : function() {
