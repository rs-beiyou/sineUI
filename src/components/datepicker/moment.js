...
export default class Moment {
  ...
  init([year, month, day, hour, minute, second]) {
    this._d = new Date(
      `${year}/${month ? month + 1 : 1}/${day || 1} ${hour || 0}:${minute ||
        0}:${second || 0}`
    );
    return this;
  }
  ...
  format(fmt) {
    ...
    str = str.replace(
      /dd|DD/,
      da.getDate() > 9 ? da.getDate().toString() : '0' + da.getDate()
    );
    str = str.replace(/d|D/g, da.getDate());

    str = str.replace(
      /hh|HH/,
      da.getHours() > 9 ? da.getHours().toString() : '0' + da.getHours()
    );
    str = str.replace(/h|H/g, da.getHours());
    str = str.replace(
      /mm/,
      da.getMinutes() > 9 ? da.getMinutes().toString() : '0' + da.getMinutes()
    );
    str = str.replace(/m/g, da.getMinutes());

    str = str.replace(
      /ss|SS/,
      da.getSeconds() > 9 ? da.getSeconds().toString() : '0' + da.getSeconds()
    );
